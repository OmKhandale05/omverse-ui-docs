/**
 * Source code for the Dashboard example — exported as a string
        ...
        {STATS.map((s) => (
          <Card key={s.label} variant="outlined">
            <CardBody>
              ...
              <p style={{ fontSize: 22, fontWeight: 600, color: 'var(--color-text-primary)', marginBottom: 4 }}>
                {s.value}
              </p>
              <p style={{ fontSize: 12, fontWeight: 500, color: s.up ? 'var(--color-success)' : 'var(--color-error)' }}>
          ...
          <CardHeader
            ...
            action={
              <ChipGroup
                mode="single"
                value={[period]}
                onChange={(vals) => { if (vals[0]) setPeriod(vals[0] as Period) }}
                size="sm"
                variant="tonal"
              >
              ...
              <tbody>
                {TRANSACTIONS.map((tx, i) => (
                  <tr
                    ...
                    style={{
                      borderTop: '0.5px solid var(--color-outline-variant)',
                      background: i % 2 === 1 ? 'var(--color-surface-variant)' : 'transparent',
                    }}
                  >
                    <td style={{ padding: '12px 24px', fontFamily: 'var(--font-mono), monospace', fontSize: 12, color: 'var(--color-text-secondary)' }}>
                      {tx.id}
                    </td>
                    <td style={{ padding: '12px 24px', color: 'var(--color-text-primary)' }}>
                      {tx.customer}
                    </td>
                    <td style={{ padding: '12px 24px', fontWeight: 500, color: 'var(--color-text-primary)' }}>
                      {tx.amount}
                    </td>
